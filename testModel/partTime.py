...
def showTimeModule(dicList):
    ...
    resultList = []
    for oneDay in dicList:
        if oneDay.month == monthBefore:
            if oneDay.day > 15:
                resultList.append(oneDay)
        if oneDay.month == monthAfter:
            if oneDay.day <= 15:
                resultList.append(oneDay)
    return resultList
